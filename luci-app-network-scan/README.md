# LuCI Network Scan (luci-app-network-scan)
...
- Runtime state stored in `/tmp/network-scan/`.
...
make package/luci-app-network-scan/compile V=s
```
...
## Runtime output
- Raw captures: `/tmp/network-scan/raw/`
- Correlated output: `/tmp/network-scan/results.json`
- Scan metadata: `/tmp/network-scan/scan_meta.json`
- Status: `/tmp/network-scan/status.json`
...
- Evidence tabs expose raw inputs (ARP, DHCP, Wi-Fi, Nmap) via `/tmp/network-scan/raw/`.
